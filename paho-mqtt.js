...
function onMessageArrived(msg){
  ...
  switch(t){
    ...
    case "smart_level/central/retrolavagem":
      ...
      if(v==="1")
        history.unshift("[INICIO] " + now);
      else
        history.unshift("[FIM] " + now);

      renderHistory();
      break;

    case "smart_level/central/retroA_status":
      document.getElementById("cfg_retroA").value = v;
      break;

    case "smart_level/central/retroB_status":
      document.getElementById("cfg_retroB").value = v;
      break;

    case "smart_level/central/timeout":
